  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (userType === 'client') {
          // Clients need to complete their profile first
        ...
        <div className="glass rounded-3xl p-8 border border-white/10">
          ...
          <div className="text-center mb-8">
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass border border-indigo-500/50 mb-4"
            >
              <Sparkles className="w-4 h-4 text-indigo-400" />
              <span className="text-sm">
                {userType === 'client' ? 'Client Account' : 'Freelancer Account'}
              </span>
            </motion.div>
            <h2 className="text-2xl font-bold mb-2">Create Your Account</h2>
            <p className="text-gray-400 text-sm">
              {userType === 'client' 
                ? 'Start managing projects and hiring talent' 
                : 'Join 10,000+ freelancers transforming their workflow'}
            </p>
          </div>
